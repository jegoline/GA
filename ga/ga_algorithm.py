...
def measure_entropy(population):
    ...
    for route in population:
        fit = int(route.get_distance())

        if not fits.has_key(fit):
            fits[fit] = 1
        else:
            fits[fit] += 1
    ...
    entropy = 0
    for f in fits.keys():
        fj = float(fits[f]) / len(fits)
        entropy += fj * math.log(fj)
    entropy = - entropy
    return entropy
...
class GA:
    ...
    def get_population(self):
        return self.population.get_all_routes()
    ...
    def measure_entropy(self):
        return measure_entropy(self.population.get_all_routes())